    ...
    public class BookEventStore : IBookEventStore
    {
        ...
        public async Task Save(BookAggregate aggregate)
        {
            using (var connection = new SqlConnection(_writeConnectionString))
            {
                ...
                var dbAggregate =
                    await connection.QueryFirstOrDefaultAsync<BookAggregate>(aggregateQuery, new { id = aggregate.Id });
                if (dbAggregate != null && dbAggregate.Version != aggregate.Version)
                {
                    throw new Exception(
                        $"Book aggregate is updated to version: {dbAggregate.Version}, current is: {aggregate.Version}");
                }
                ...
                try
                {
                    ...
                    if (dbAggregate == null)
                    {
                        var insertAggregateCmd =
                            "INSERT INTO post_write.dbo.BookAggregate(Id, Version, CreatedDate, ModifiedDate) VALUES (@Id, @Version, @CreatedDate, @ModifiedDate);";
                        await connection.ExecuteAsync(insertAggregateCmd, aggregate, transaction: trans);
                    }
                    else
                    {
                        var updateAggregateCmd =
                            "UPDATE BookAggregate SET Version=@Version, ModifiedDate=@ModifiedDate WHERE Id=@Id;";
                    ...
                    var insertEventCmd =
                        "INSERT INTO post_write.dbo.BookEvent (Id, AggregateId, Version, CreateDate, [Data], [Type]) VALUES(@Id, @AggregateId, @Version, @CreateDate, @Data, @Type);";
                    foreach (var se in serealizedEvents)
                    {
